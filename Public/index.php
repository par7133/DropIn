 ...
 */
 ...
 function colorCats($s) {
   $pattern = '/' . APP_CATS . '/';
   $rep = '<span style="color:red">${1}</span>';
   return preg_replace($pattern, $rep, $s);     
 }
 ...
 function showHistory() {
   ...
   foreach($signHistoryCopy as $val) {
     ...
     if ($contextType === PERSONAL_CONTEXT_TYPE) {
       ...
       $adminFnc = PHP_STR;
       if ($myflag === "u") {
         $adminFnc = "<a href='#' onclick=\"confSign('" . $myid . "')\"><img src='/DI_res/confirm.png' style='width:36px;'></a>";
       } else {
         $adminFnc = "<a href='#' onclick=\"delSign('" . $myid . "')\"><img src='/DI_res/del.png' style='width:36px;'></a>";
       }    
       ...
       echo("<span class='data-title' onclick='openEx(\"" . $m . "\");'>".$mytitle."</span>");
       echo("<div id='hheader".$m."' class='exerpt'>".colorCats(HTMLencode($myheader, true))."</div>");
       echo("<div id='eexerpt".$m."' class='exerpt'>".HTMLencode($mydesc, true)."</div>");       
       echo("</td>");
     ...
     // If I'm not in admin
       ...
       if ($myflag !== "u") {
         ...
         echo("<span class='resli' onclick='openEx(\"" . $m . "\");'>");
         echo("<span class='this-title'>".$mytitle."&nbsp;<span class='tag'>[$mycat]</span></span>");
 ...
 function upload() {
   ...
   if ($t !==PHP_STR) {
       ...
       $fileContent = explode(PHP_EOL,$t);

       if (left($fileContent[0],8) !== "#title: ") {
          return;
       }
...
<!--
 ...
 -->
  ...
  <script src="/DI_js/index-js.php" type="text/javascript" defer></script>
  
  <link href="/DI_css/bootstrap.min.css" type="text/css" rel="stylesheet">
  <link href="/DI_css/style.css?r=<?PHP echo(time());?>" type="text/css" rel="stylesheet">
...
<div id="sidebar">
    ...
    <br><br>
    <img id="genius" src="/DI_res/HLgenius.png" alt="HL Genius" title="HL Genius">
    &nbsp;<br><br>
  ...
  <div style="width:100%; padding: 8px; text-align:center; font-size:26px; border:0px solid red;">
    ...
    <?php if (APP_DEFAULT_CONTEXT === "PRIVATE"): ?>
     ...
     <div id="content-header">
      ...
      <?php else: ?>
        ...
        <img src="/DI_res/dnd2.gif">
        </div>
        </div>

        <div id="template-img">
        <div id="templated">
        <a href="/template.txt"><img src="/DI_res/template.png"></a>
        </div>
        </div>
        ...
        <button id="modalButtonNew" type="button" class="btn btn-primary" style="display:none;" data-toggle="modal" data-target="#modalNew">Button #1</button>

        <div class="modal" tabindex="-1" role="dialog" id="modalNew">
          <div class="modal-dialog modal-lg my-modal-dialog" style="left:10%;" role="document">
            <div class="modal-content my-modal-content" style="height:550px;border:3px solid #000000; overflow:scroll;">
            ...
            <div class="modal-toolbox my-modal-toolbox" style="float:left;">
    ...
    <?php else: ?>
      ...
      <?php if ($contextType === PUBLIC_CONTEXT_TYPE): ?>
        ...
        <option value=""></option>
        <?PHP foreach ($aCats as $vcat): ?>
        <option value="<?PHP echo($vcat); ?>" <?PHP echo(($cat===$vcat)?"selected":"");?>><?PHP echo($vcat);?></option>
        <?PHP endforeach;?>
        </select>
        </div>              
